...
function buildChartData(
  ...
  // Pre-parse timestamps in a single O(N) pass to avoid O(N log N) Date.parse calls during sort
  const usageWithTimestamps = usageTimeline.map((usage) => ({
    usage,
    parsedTimestamp: Date.parse(usage.timestamp),
  }))

  const sortedUsage = usageWithTimestamps.sort(
